...
export function MultiSelectMembers({ selectedIds, onChange, error }: MultiSelectMembersProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        console.warn('Failed to load users from live backend in MultiSelectMembers, using high-fidelity local seed list:', err);
  ...
  const handleRemoveChip = (userId: number, event: React.MouseEvent) => {
  ...
  return (
    <div className="w-full" ref={containerRef}>
      <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
        Assign Members <span className="text-rose-500" aria-hidden="true">*</span>
      </label>
        ...
        <div className="flex flex-wrap gap-2 mb-2.5 max-h-[120px] overflow-y-auto p-1.5 border border-gray-100 dark:border-gray-800 rounded-lg bg-gray-50/40 dark:bg-gray-900/10">
          {selectedUsers.map((user) => (
            ...
            >
              ...
              <div className={classNames('w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold', getAvatarTheme(user.name))}>
                {getInitials(user.name)}
              </div>
              <span>{user.name}</span>
              <button
                type="button"
                onClick={(e) => handleRemoveChip(user.id, e)}
        ...
        >
          {isOpen ? (
            ...
            <span className={classNames(
              ...
              selectedUsers.length === 0 ? 'text-gray-400 dark:text-gray-500' : 'text-gray-800 dark:text-gray-200 font-medium'
            )}>
              {selectedUsers.length === 0
