...
import { BugDiscussionMessage } from '@/lib/api/bugs';

export function BugDiscussionPanel({ bugId, currentUserId }: { bugId: number, currentUserId?: number | string }) {
  const [messages, setMessages] = useState<BugDiscussionMessage[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await apiClient.get(`/bugs/${bugId}/discussions`);
        setMessages(res.data as BugDiscussionMessage[]);
        
        // Mark as read immediately
        await apiClient.post(`/bugs/${bugId}/discussions/read`, {});
      } catch (err) {
        console.error('Error fetching bug discussions', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    socketRef.current.on('connect', () => {
      socketRef.current?.emit('join_bug_room', { bugId });
    });

    socketRef.current.on('new_message', (message: BugDiscussionMessage) => {
    ...
    return () => {
      socketRef.current?.emit('leave_bug_room', { bugId });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isTyping) {
      ...
      socketRef.current?.emit('bug_typing', { bugId, userName: 'Someone' });
    }
    ...
    typingTimeoutRef.current = setTimeout(() => {
      setIsTyping(false);
      socketRef.current?.emit('stop_bug_typing', { bugId });
    }, 1000);
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post(`/bugs/${bugId}/discussions`, { message: messageText });
    } catch (err) {
      console.error('Failed to send bug message', err);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center py-10 flex flex-col items-center">
            ...
            <p className="text-sm text-gray-400 mt-1">Start the conversation about this bug.</p>
          </div>
        ) : (
          messages.map((msg) => {
